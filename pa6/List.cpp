...
int List::findPrev(ListElement x) {
    while (beforeCursor != frontDummy) {
        if (beforeCursor->data == x) {
            movePrev(); // Properly move the cursor to the found element
            return pos_cursor;
        }
        movePrev();
    }
...
void List::cleanup() {
    ...
    int originalPosition = pos_cursor;  // Store the original cursor position
    ...
    while (curr != backDummy) {
        ...
        while (checker != backDummy) {
            if (checker->data == curr->data) {
                ...
                // Adjust the position cursor if we are deleting nodes that affect the cursor
                if (checker == beforeCursor) {
                    beforeCursor = beforeCursor->prev;
                    pos_cursor--;
                } else if (checker == afterCursor) {
                    afterCursor = afterCursor->next;
                }
    ...
    // Adjust the position cursor to be the original element if possible
    if (originalPosition > num_elements) {
        pos_cursor = num_elements;
        beforeCursor = backDummy->prev;
        afterCursor = backDummy;
    } else {
